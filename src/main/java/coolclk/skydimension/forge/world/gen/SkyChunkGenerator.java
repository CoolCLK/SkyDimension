...
public class SkyChunkGenerator extends ChunkGenerator {
    public static final Codec<SkyChunkGenerator> CODEC = RecordCodecBuilder.create((instance) -> {
        ...
        }), DimensionSettings.CODEC.fieldOf("settings").forGetter((generator) -> {
    ...
    public SkyChunkGenerator(BiomeProvider p_i241975_1_, long p_i241975_2_, Supplier<DimensionSettings> p_i241975_4_) {
        this(p_i241975_1_, p_i241975_1_, p_i241975_2_, p_i241975_4_);
    }

    private SkyChunkGenerator(BiomeProvider biomeSource, BiomeProvider runtimeBiomeSource, long seed, Supplier<DimensionSettings> settings) {
    ...
    protected BlockState generateBaseState(double p_236086_1_, int height) {
        BlockState blockstate;
        if (p_236086_1_ > 0.0D) {
            blockstate = this.defaultBlock;
        } else if (height < this.getSeaLevel()) {
    ...
    public void buildSurfaceAndBedrock(@Nonnull WorldGenRegion region, @Nonnull IChunk chunk) {
        ...
        for(int i1 = 0; i1 < 16; ++i1) {
            for(int j1 = 0; j1 < 16; ++j1) {
                ...
                region.getBiome(blockpos$mutable.set(k + i1, i2, l + j1)).buildSurfaceAt(sharedseedrandom, chunk, k1, l1, i2, d1, this.defaultBlock, this.defaultFluid, this.getSeaLevel(), region.getSeed());
            }
        }
    }

    @Override
    public void fillFromNoise(@Nonnull IWorld world, @Nonnull StructureManager manager, IChunk chunk) {
    ...
    @Override
    @Nonnull
    public List<MobSpawnInfo.Spawners> getMobsAt(@Nonnull Biome biome, @Nonnull StructureManager manager, @Nonnull EntityClassification classification, @Nonnull BlockPos pos) {
    ...
    @Override
    public void spawnOriginalMobs(@Nonnull WorldGenRegion p_230354_1_) {
    ...
    static {
        Registry.register(Registry.CHUNK_GENERATOR, new ResourceLocation(SkyDimension.MOD_ID, "sky"), SkyChunkGenerator.CODEC);
    }
}
